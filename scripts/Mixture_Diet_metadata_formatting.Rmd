...
modified_sample_data <- function(asv_data) {
  asv_data %>%
    ...
    select(samplename, time, creek, station, biol, tech) %>%
...
run_sample_data <- left_join(run_sample_data, station_map, by=c("creek","station"))  

run_sample_data %>% 
  mutate(model_out_name = paste(time,creek,station_num,biol,tech, sep = "."),
         model_out_name2 = paste(time,creek,station_idx,biol,tech, sep = ".")) -> run_sample_data

CF_combined_md_mod <- left_join(CF_combined_md,run_sample_data, by="SampleID")
