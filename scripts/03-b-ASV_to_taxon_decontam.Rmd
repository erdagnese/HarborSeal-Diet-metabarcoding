...
```{r}
asv <- read.biom(here("data","CorrectionBias","tourmaline-18S_CB2_13122024","02-output-dada2-pe-unfiltered","00-table-repseqs", "table.biom"), tree = FALSE, prune = FALSE)
# mifish
asv <- read.biom(here("data","CorrectionBias","tourmaline-MiFish-20230130","02-output-dada2-pe-unfiltered","00-table-repseqs", "table.biom"), tree = FALSE, prune = FALSE)

tax <- read.csv(here("data","CorrectionBias","ComBayRun2_18S_final_taxonomy_table.csv"), header = TRUE)
# for mifish
tax <- read_tsv(here("data","MiFish_final_Taxonomy.tsv"))
tax %>% rename(c(featureid = "Feature ID", final_taxonomy = Taxon)) -> tax

metadata <- read.table(here("data","tourmaline-18S_CB2_13122024","00-data","metadata.tsv"), sep = "\t", header = T)
