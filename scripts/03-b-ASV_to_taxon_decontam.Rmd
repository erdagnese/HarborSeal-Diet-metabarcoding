...
tax <- read.csv(here("intermediate-files","CorrectionBiasRun","Prey18S","final_taxonomy_CorrectionBias_20250724_183741.csv"), header = TRUE)


metadata <- read.table(here("data","CorrectionBias","tourmaline-CorBias_18S_16072025","00-data","metadata.tsv"), sep = "\t", header = T)

# for mifish
tax <- read_tsv(here("intermediate-files","CorrectionBiasRun","MiFish","final_taxonomy_CorBias_MiFish.tsv"))
...
metadata <- read.table(here("data","CorrectionBias","tourmaline-CorBias_MiFish_16072025","00-data", "metadata.tsv"), sep = "\t", header = T)

tax <- read_tsv(here("intermediate-files","Wild_CB_Samples","final_taxonomy_Wild_CB2_MiFish.tsv"))
MiFishBiom <- read_biom(here("data","Wild_CB_Samples","tourmaline-MiFish_CB2_12122024","02-output-dada2-pe-unfiltered","00-table-repseqs", "table.biom"), tree = FALSE, prune = FALSE)


metadata <- read.table(here("data","Wild_CB_Samples","tourmaline-MiFish_CB2_12122024","00-data", "metadata.tsv"), sep = "\t", header = T)
```
...
posCtrlasv <- posctrl_decontam_data(asv, c("PosCtrl_1","Plate3_H12"))
...
#tax %>% dplyr::rename("final_taxonomy"="Taxon") %>% dplyr::rename("featureid"="feature_id") -> tax
modified_tax <- resolution_MiFish_tax(tax)
tax <- modified_tax
...
write.csv(taxon_table_wide, file = here::here("intermediate-files", "CorrectionBiasRun","CorBias_18S_taxon_decontam_asv_table.csv") , row.names = F)

write.csv(taxon_table_wide, file = here::here("intermediate-files", "CorrectionBiasRun","CorBias_MiFish_taxon_decontam_asv_table.csv") , row.names = F)
#write.csv(taxon_table_wide, "Test_MiFish_taxon_decontam_table.csv", row.names = F)
write.csv(taxon_table_wide, file = here::here("intermediate-files", "Wild_CB_Samples","Wild_CB2_MiFish_taxon_decontam_asv_table.csv") , row.names = F)
```
