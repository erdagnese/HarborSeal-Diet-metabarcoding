...
```{r}
mock_data <- read.csv(here("input-files","Mock_communities_MiFish_metadata.csv"), header=T, check.names = FALSE)
```
...
```{r}
mock_data %>% 
  rename(taxon_assign = "MiFish_taxon_assign") %>% 
  rename(species = "Species") %>% 
  select(species,taxon_assign, Even_prop, Even_mass_ng, SK1_prop, SK1_mass_ng, SK2_prop, SK2_mass_ng) %>% 
  mutate(Community = "PvPrey")-> mock_data
...
```{r}
asv_data <- read.csv(here("intermediate-files","CorrectionBiasRun","CorBias_MiFish_taxon_decontam_asv_table.csv"), header = T, check.names = F)

asv_meta <- read.csv(here("input-files","CorrectionBias_run_metadata.csv"),header = T, check.names = F)

asv_wild_data <- read.csv(here("intermediate-files", "Wild_CB_Samples", "Wild_CB2_MiFish_taxon_decontam_asv_table.csv"), header = T, check.names = F)

wild_md3 <- read.csv(here("input-files","FULL_CBrun2_metadata.csv"), header = T)

expected_prop_mapTM <- read.csv(here("input-files","TM_master_input_metadata.csv"), header = T)
expected_prop_mapCtrF <- read.csv(here("input-files","PvCF_master_scat_metadata.csv"), header = T)
...
# Identify the row(s) with the specific taxonomy you want to modify - this is based off of the tissue mixture and seeing that in the mock there are missing reads to O. keta that should be in there.
target_taxonomy <- "Eukaryota;Chordata;Actinopteri;Salmoniformes;Salmonidae;Oncorhynchus"
target_rows <- asv_data$final_taxonomy == target_taxonomy
asv_data$final_taxonomy[target_rows] <- "Eukaryota;Chordata;Actinopteri;Salmoniformes;Salmonidae;Oncorhynchus;Oncorhynchus keta"
...
filtered_asv_data <- asv_data %>%
  ...
  # Group by both taxonomy and sampleid, then sum counts
...
mock_asv_data <- mock_asv_data %>%
  ...
  separate(
    samplename,
    into = c("sampletype", "marker", "commmix", "tech"),
    sep = "_",
    remove = FALSE
  ) %>%
  ...
  mutate(
    ...
    community = paste(sampletype, marker, commmix, sep = "_")
  )
...
ids_to_match <- unique(long_mock_data$ID_MiFish)
...
# Filter and merge
merged_data <- mock_asv_data %>%
  filter(final_taxonomy %in% valid_taxa) %>%
  mutate(commtype = "PvPrey") %>% 
  mutate(community = paste(commtype, commmix, sep = "_")) %>% 
  inner_join(long_mock_data, by = "community") %>%
  filter(str_detect(final_taxonomy, ID_MiFish)) 


```
Let's clean up the mock data of the species that hardly appeared in the mock if at all
```{r}
#let's remove the species that clearly didn't really make it into the mocks consistently
merged_data %>% 
  filter(ID_MiFish != "Liparis fucensis") %>%
  filter(ID_MiFish != "Diaphus theta") %>%
  filter(ID_MiFish != "Gobiesox maeandricus") %>%
  filter(ID_MiFish != "Chitonotus pugetensis") %>% 
  filter(ID_MiFish != "Hippoglossoides elassodon") %>%
  filter(ID_MiFish != "Platichthys stellatus") %>%
  filter(ID_MiFish != "Oncorhynchus kisutch") %>%
  filter(ID_MiFish != "Oncorhynchus tshawytscha") %>%
  filter(ID_MiFish != "Oncorhynchus keta") %>%
  filter(ID_MiFish != "Porichthys notatus") %>%
  filter(ID_MiFish != "Anoplarchus insignis") %>% 
  filter(ID_MiFish != "Lumpenus sagitta") %>% 
  filter(ID_MiFish != "Lepidopsetta bilineata") %>% 
  filter(ID_MiFish != "Pholis laeta") -> merged_data
```
...
# Example usage:
p1 <- plot_taxonomy_proportions(merged_data, 
                               plot_title = "Initial Mock Check")

print(p1)
```
Okay that looks pretty normal 
...
asv_meta %>% 
  filter(str_detect(Plate_ID, "MiFish")) %>% 
  filter(region %in% c("ControlFeeding","TissueMix")) -> asv_meta_MF_samples


# First, get the sample IDs you want to keep from the metadata
samples_to_keep <- asv_meta_MF_samples$sampleid
...
samplesMF_asv_data <- filtered_asv_data %>%
  left_join(asv_meta_MF_samples, by = "sampleid") %>% 
  select(samplename,region,final_taxonomy,count)

```

formatting the CB metadata then asv data to select samples that have enough reads to the mock species to work in the model
```{r}
library(tidyverse)
library(stringr)
...
merged_data%>% 
  select(ID_MiFish, community, tech, count,start_conc_ng) %>% 
  mutate(Cycles = 39) %>% 
  rename("nReads" = "count") -> mock_MF_input

input_mock_comm <- mock_MF_input %>% 
  filter(., Cycles ==39) %>% 
  ...
  dplyr::select(Community=community, Tech_Rep=tech,species=ID_MiFish, nReads,totReads,propReads,start_prop, N_pcr_mock=Cycles) %>% 
...
input_mock_comm %>% 
  ...
  ungroup() %>% 
  mutate(station = station_idx) -> input_mock_comm_RDS

#save for future use
saveRDS(input_mock_comm_RDS, here("intermediate-files","PvPrey_MiFish_mock_community_data.RDS") )
...
```{r}
# Example usage:
p3 <- plot_proportion_comparison(input_mock_comm_RDS,existing_prop_col = "propReads",
...
saveRDS(combined_mock2, here("intermediate-files","input_PvPrey_MiFish_mock_data.RDS"))

combined_mock2 <- readRDS(here("intermediate-files","input_PvPrey_MiFish_mock_data.RDS"))
```
...
sample_data <- modified_sample_data(samplesMF_asv_data)
...
filtered_wild_samples <- filter_sample_data_by_metrics(
  ...
  min_prop_mock = 0.2, min_mock_species = 2, min_total_reads = 5000
)
...
#save for future use
saveRDS(combined_sample_data, here("intermediate-files","PreyMiFish_ALLObsSamples_data_20250828.RDS"))
...
#save for future use
saveRDS(combined_sample_data_filtered, here("intermediate-files","PreyMiFish_CF_TM_Wild_input_Obs_data.RDS"))

combined_sample_data_filtered <- readRDS(here("intermediate-files","PreyMiFish_CF_TM_Wild_input_Obs_data.RDS"))
```
...
saveRDS(station_mapping, here("intermediate-files","PreyMF_observationData_stationMapping.RDS"))
station_mapping <- readRDS(here("intermediate-files","PreyMF_observationData_stationMapping.RDS"))
```
...
qmdata <- format_metabarcoding_data(input_metabarcoding_RDS,input_mock_comm_RDS,
                          ...
                          reference_species = "Alosa sapidissima")
...
write_rds(ML_out, here("intermediate-files","MiFish_test1_08282025_ML_out.RDS"))
...
ggsave(here("Figures","CorrectionBiasRun","20250828_MF_ctrlfeeding_proportions_after_qm_ML.png"), width = 18, height = 10, units = "cm")
...
ggsave(here("Figures","20250828_MF_TM_proportions_after_qm_ML.png"), width = 18, height = 10, units = "cm")
...
```{r}
QM_bayes <- function(stanmodelname, stan_metabarcoding_data, NCHAINS = 4, WARMUP = 1000, ITER = 5000){
  require(tidyverse)
  require(rstan)
  rstan_options(auto_write = TRUE)
  options(mc.cores = parallel::detectCores())
...
saveRDS(bayes_out, here("intermediate-files","CorrectionBiasRun","MiFish","model_out","20250828_MFbayesout_ControlFeedingTMWildScats.RDS"))

summaryout <- summary(bayes_out$Bayes_modelfit)$summary
write.csv(summaryout, here("intermediate-files","CorrectionBiasRun","MiFish","model_out","20250828_MFbayesout_ControlFeedingTMWildScats_summary.csv"))

bayes_out <- readRDS( here("intermediate-files","CorrectionBiasRun","MiFish","model_out","20250828_MFbayesout_ControlFeedingTMWildScats.RDS"))
...
The model is struggling. The very low n_eff and high Rhat values for a few species in your bayes_out printout are major red flags. 

```{r}
# 4. Run the model with the COMBINED mock data and the NEW station_mapping
bayes_out_2 <- run_QM_model(
...
saveRDS(int_samp_small_tibble, here("intermediate-files","CorrectionBiasRun","MiFish","model_out","20250829_MFbayesout_ControlFeedingTMWildScats_int_samp_small_tibble.RDS"))
...
saveRDS(plotting_data, here("intermediate-files","CorrectionBiasRun","MiFish","model_out","20250828_MFbayesout_ControlFeedingTMWildScats_qm_data_plotting.rds"))
...
saveRDS(mean_estimates, here("intermediate-files","CorrectionBiasRun","MiFish","model_out","20250828_MFbayesout_ControlFeedingTMWildScats_qm_mean_estimates.rds"))
saveRDS(summary_stats, here("intermediate-files","CorrectionBiasRun","MiFish","model_out","20250828_MFbayesout_ControlFeedingTMWildScats_qm_samp_small_summary_stats.rds"))
saveRDS(species_summary, here("intermediate-files","CorrectionBiasRun","MiFish","model_out","20250828_MFbayesout_ControlFeedingTMWildScats_qm_species_summary.rds"))
saveRDS(full_posterior, here("intermediate-files","CorrectionBiasRun","MiFish","model_out","20250828_MFbayesout_ControlFeedingTMWildScats_full_posterior.rds"))
